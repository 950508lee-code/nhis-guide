...
import { BarChart3, ArrowLeft, Users, MapPin, ChevronRight, X, Lock } from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-1 px-5 py-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          {stats.totalUsers === 0 ? (
            <div className="text-center py-8 text-gray-500">아직 이용자 데이터가 없습니다</div>
          ) : (
            <div className="space-y-3">
              {monthlyData.map((data) => (
                <div key={data.month} className="flex items-center gap-3">
                  <span className="text-sm text-gray-600 w-12">{formatMonth(data.month)}</span>
                  <div className="flex-1 bg-gray-100 rounded-full h-6 overflow-hidden">
                    <div
                      className="bg-blue-500 h-6 rounded-full transition-all duration-500 flex items-center justify-end pr-2"
                      style={{ width: `${Math.max((data.count / maxMonthlyCount) * 100, data.count > 0 ? 15 : 0)}%` }}
                    >
                      {data.count > 0 && (
